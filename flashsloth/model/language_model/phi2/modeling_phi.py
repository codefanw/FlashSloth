...
from flashsloth.model.learnable_token import LearnableToken
from flashsloth.model.lqformer import LQFormer
try:
...
class PhiModel(PhiPreTrainedModel):
    ...
    def __init__(self, config: PhiConfig):
        ...
        self.learnable_tokens = LearnableToken(9, self.config.hidden_size) 
        self._use_flash_attention_2 = config._attn_implementation == "flash_attention_2"

        self.gradient_checkpointing = False
        self.num_fusionlayer = 1
        self.lqformer = LQFormer(config=config, num_layers=self.num_fusionlayer)
    ...
    def get_learnabletoken(self):
        return self.learnable_tokens()
    
    def is_all_empty(self, nested_list):
        return all(len(sublist) == 0 for sublist in nested_list)
    ...
    def forward(
        ...
        insert_place: Optional[List[List]] = [[]],
        image_features: Optional[torch.FloatTensor] = None,
        learnable_token_len: Optional[int] = 6,
        modal: Optional[List[int]] = [0],
        question_token_ranges: Optional[List[List[Tuple[int, int]]]] = [[]]
        ...
        for idx, decoder_layer in enumerate(self.layers):
            all_combined_learnabletokens = []
            all_combined_imagetokens = []
            ...
            if hidden_states.shape[1]!=1 and idx+1 == 8 and image_features is not None:
                ...
                for batch_idx, (learnable_indices, question_indices) in enumerate(zip(insert_place, question_token_ranges)):
                    for q_start_idx, q_end_idx in question_indices:
                        max_text_len = max(max_text_len, q_end_idx - q_start_idx)
                combined_learnabletokens = []
                ...
                all_combined_textmasks = torch.cat(combined_textmasks, dim=0)
                # Pass tokens and masks through LQFormer
                lqformer_outputs = self.lqformer(all_combined_learnabletokens, all_combined_imagetokens, all_combined_texttokens, text_mask=all_combined_textmasks)
