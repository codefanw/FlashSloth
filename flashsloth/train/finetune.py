...
from flashsloth.mm_utils import tokenizer_image_token, process_images_hd
...
def preprocess_phi2(
    ...
    for i, source in enumerate(sources):
        ...
        for j, sentence in enumerate(source):
            ...
            conv.append_message(role, sentence["value"])
        conversations.append(conv.get_prompt())
        if "ASSISTANT" in conversations[0]:
            conversations = [text.replace("ASSISTANT",LEARNABLE_TOKEN +" ASSISTANT") for text in conversations]
        else:
            ValueError("learnable token add failure.")

    # Tokenize conversations

    local_rank = int(os.environ["LOCAL_RANK"])  # Get the rank of each GPU
    save_dir = "debug_conversations"
    os.makedirs(save_dir, exist_ok=True)
    with open(os.path.join(save_dir, f"conversation_step_{local_rank}.txt"), "w") as f:
    ...
    for conversation, target in zip(conversations, targets):
        ...
        target[target == LEARNABLE_TOKEN_INDEX] = IGNORE_INDEX
        ...
        try:
            ...
            if 'image' in sources[0]:
                ...
                if self.data_args.image_aspect_ratio == 'pad':
                    ...
                    image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
                elif self.data_args.image_hd:
                    image = process_images_hd(image, self.data_args.image_processor, None)
                else:
                    image = processor.preprocess(image, return_tensors='pt')['pixel_values'][0]
            ...
            if 'image' in self.list_data_dict[i]:
                ...
                if self.data_args.image_hd:
                    data_dict['images'] = torch.zeros(5, 3, *crop_size.values())
                else:
...
def train():
    ...
    model.config.image_hd = data_args.image_hd
