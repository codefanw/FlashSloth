...
if local_rank != "0" and local_rank is not None:
    errfile = open(f'logs/error_log_rank{local_rank}.txt','w')
...
from flashsloth.constants import IGNORE_INDEX, IMAGE_TOKEN_INDEX, DEFAULT_IMAGE_TOKEN, DEFAULT_IM_START_TOKEN, DEFAULT_IM_END_TOKEN, LEARNABLE_TOKEN_INDEX,LEARNABLE_TOKEN
from torch.utils.data import Dataset
from flashsloth.train.llava_trainer import LLaVATrainer

from flashsloth import conversation as conversation_lib
from flashsloth.model import *
from flashsloth.model.multimodal_encoder.builder import build_vision_tower
from flashsloth.mm_utils import tokenizer_image_token
...
def find_all_linear_names(model):
    ...
    # if 'linear' in lora_module_names: # needed for 16-bit
    #     lora_module_names.remove('linear')
    logger.info(f'lora_module_names: {list(lora_module_names)}')
...
def preprocess_phi2(
    ...
    for conversation, target in zip(conversations, targets):
        ...
        for i, rou in enumerate(rounds):
            ...
            if has_image:
                round_len = len(tokenizer_image_token(rou, tokenizer)) + 1 # add 1 for the sep </s>
                instruction_len = len(tokenizer_image_token(parts[0], tokenizer)) - 1 # minus '_' after ':'
            else:
...
def preprocess_plain(
    ...
    for source in sources:
        ...
        conversation = source[0]['value'] + source[1]['value']+" " +LEARNABLE_TOKEN + conversation_lib.default_conversation.sep
    ...
    for target, source in zip(targets, sources):
        ...
        learnable_token_position = target.tolist().index(LEARNABLE_TOKEN_INDEX)
        target[learnable_token_position] = IGNORE_INDEX
    # print(targets[0])
...
def train():
    ...
    # FIXME: It's a hack to build the vision tower, i.e., Siglip-SO400M, 
    # before the LLM backbone is built. Otherwise, there is a weight loading 
    # error when deepspeed ZeRO-3 is enabled.
    ...
    if model_args.vision_tower is not None:
        if 'phi2' in model_args.model_name_or_path.lower() or 'phi-2' in model_args.model_name_or_path.lower():
            ...
            # config = FlashSlothConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            config = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            model = FlashSlothForCausalLM.from_pretrained(
            ...
            if not model_args.model_name_or_path.endswith('/'):
                model_args.model_name_or_path = model_args.model_name_or_path + '/'
            # config = FlashSlothQwen2Config.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            config = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            model = FlashSlothQwen2ForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                # attn_implementation='flash_attention_2',
            ...
            if not model_args.model_name_or_path.endswith('/'):
                model_args.model_name_or_path = model_args.model_name_or_path + '/'
            # config = FlashSlothPhi3Config.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            config = AutoConfig.from_pretrained(model_args.model_name_or_path, trust_remote_code=True)
            model = FlashSlothPhi3ForCausalLM.from_pretrained(
                model_args.model_name_or_path,
                config=config,
                cache_dir=training_args.cache_dir,
                attn_implementation='flash_attention_2',
    ...
    if model_args.vision_tower is not None:
        ...
        if training_args.freeze_mm_mlp_adapter:
            logger.info("Freezing mm_mlp_adapter")
            for p in model.get_model().mm_projector.parameters():
    ...
    model.model.learnable_tokens.reset_parameters()  
    ...
    for name, param in model.named_parameters():
        ...
        if 'lqformer' in name or "pooling" in name or "learnable" in name:
            param.requires_grad = True
    for name, param in model.named_parameters():
